...
### 9.5 Metrics, tracing, and SLA enforcement

Each search call site should be wrapped in a Micrometer `Timer` and traced via Micrometer
Tracing so we can back the 6-second SLA with evidence:

```java
@Service
@RequiredArgsConstructor
class MetricsAwareMandateSearchService implements MandateSearchService {

    private final MandateSearchService delegate;
    private final MeterRegistry meterRegistry;
    private final Tracer tracer;

    @Override
    public List<MandateHit> searchMandatesByText(String query, int limit) {
        return Timer.builder("planlibrary.search.latency")
            .tag("kind", "lexical")
            .tag("index", "mandates_lexical")
            .publishPercentiles(0.5, 0.95, 0.99)
            .register(meterRegistry)
            .record(() -> {
                Span span = tracer.currentSpan().name("search.mandates.lexical");
                try (var ws = tracer.withSpan(span.start())) {
                    return delegate.searchMandatesByText(query, limit);
                } finally {
                    span.end();
                }
            });
...
Expose `/actuator/prometheus` and scrape from the existing observability stack. Tag the
metrics with: query kind, index name, cluster profile, and 6-second SLA breach counter
(`planlibrary.search.sla.breach`).

Add a custom Atlas profiler correlation ID by injecting `traceId` into a comment in the
aggregation - this is a known good way to stitch Java traces to Atlas `system.profile`
documents until a dedicated Mongo OTel exporter is on the roadmap.
...
Java-side metrics for the SLA:

```
curl localhost:8080/actuator/prometheus | grep planlibrary_search
```
